...
\pagestyle{empty}
	...
	\vfill
	{\Large\LaTeX ified Revision - Alpha version 0.1.1\\\today }\par
	\vspace{0.2 cm}
...
Once a wound has been treated via \nameref{skill:Surgery}~so that no more Blood Loss can occur, HLT lost to Blood Loss will begin replenishing at a rate of 1 HLT per week.
...
Characters can attempt to staunch the loss of blood with the \nameref{skill:Surgery} skill and whatever materials are at hand. A wound must be targeted with this skill which, if successful, terminates all Blood Loss from that wound.
