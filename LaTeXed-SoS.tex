...
\usepackage{tabu,longtable,ctable} %Ctable is for ???
\usepackage{fancyhdr}
\usepackage{lettrine}
...
{\vspace{8pt}\centering ------- \decofourleft\decofourright ------- \par}
...
\usepackage{multirow} %For cells in tabu spanning multiple rows
...
\begin{quote}
	\centering
\emph{“When I was in Illegon, I had an apprentice. I forget his name. A hundred times I showed him the right way to parry, and a 
...
\section{Character Creation}
When purchasing Proficiencies with PCP, you receive a number of Proficiency Points as indicated in \autoref{tab:PCPInvestment}~in \autoref{ch:charcreation}, and may then use those points to purchase and advance one or more 
Proficiencies.
...
\emph{Action Initiative} is the order in which Maneuvers declared in an Action resolve. Action Initiative, or just Initiative, is held by one character against another character that they are
...
\begin{quotation}
	\emph{Goofus has a short sword, whereas Gallant has a pike. Goofus’ Short Sword has S Reach, whereas Gallant’s pike has LL Reach. If Goofus wants to attack Gallant, that’s five (5) stages of Reach he would need to pay for to try and make the attack (5 CP). If the attack succeeds, then he would move the fight to S Reach, since he got close enough to hit.}

	\emph{At S Reach, Gallant is now the one at the disadvantage, and Attack AND Defense Maneuvers using his weapon each suffer a 5 CP Activation Cost. He could still Void incoming attacks without penalty, since these do not use his now cumbersome weapon.}
\end{quotation}
\subsubsection{Range}
...
\begin{quotation}
	\emph{Bobby is Outflanking his enemy, Bill, and declares a Torso Swing on Bill, and hits. The Hit Location roll determines that the attack hits the Chest. However, because Bobby is making a Back Attack, it actually hits Bill’s Upper Back.}
\end{quotation}
...
\subsection{Explosions} %EXPLOSIONS
...
\subsection{\label{sec:Pain}Pain}---\quad
...
character suffers a Halved Combat and Mobility pool, and gains Fatigue at double the normal rate). 

Every 24 hours, the character must roll a Health Check at RS equal to the Infection Level of the Wound. 
This roll can be modified with the \nameref{skill:Surgery} skill, as detailed on Table 7.2, and this will greatly increase a 
...
Alternatively, flip a coin or roll for it. GMs, be sure to be consistent about this sort of thing!
\newpage
\section{Target Zone Hit Locations} \vspace{-15pt} %ADD BACK ATTACKS?
\begin{table}[!hb]
...
\begin{table}[hb] %Upper Head - Cutting
	...
	\begin{tabu} to \linewidth {X[l] *{3}{X[-2,c]} X[2]X[4]}
\rowfont[c]{} Wound Level &Stun &Pain &Bleed &Special &Description\\\toprule
...
\begin{table}[hb] %Upper Head - Piercing
	...
	\begin{tabu} to \linewidth {X[c] *{3}{X[-2,c]} X[3]X[4]}
...
\begin{table}[hb] %Upper Head - Bludgeoning
	...
	\begin{tabu} to \linewidth {X[c] *{3}{X[-2,c]} X[2]X[4]}
Wound Level &Stun &Pain &Bleed &Special &Description\\\toprule
...
\begin{table}[!hb] %Elbow - Piercing
	...
	\begin{tabu} to \linewidth {X[c] *{3}{X[-2,c]} X[2]X[4]}
Wound Level &Stun &Pain &Bleed &Special &Description\\\toprule
Level 1& 1& 4& 3& \nameref{par:Disarm vs} RS 1 &Painful stab in elbow. May drop item.\\
Level 2& 2& 6& 6& \nameref{par:Disarm vs} RS 3 &Deeper penetration, nerve injury, may drop item.\\
Level 3& 3& 10& 10& \nameref{par:Autodrop}, \newline
...
\begin{table}[!hb] %Lower Back - Cutting
	\caption{Lower Back - Cutting} \vspace{-8pt}
...
\begin{table}[!hb] %Lower Back - Piercing
	\caption{Lower Back - Piercing} \vspace{-8pt}
...
\begin{table}[!hb] %Lower Back - Unarmed
	\caption{Lower Back - Bludgeoning} \vspace{-8pt}
...
\begin{table}[!hb] %Lower Back - Unarmed
	\caption{Lower Back - Unarmed} \vspace{-8pt}
