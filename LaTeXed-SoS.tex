...
\section{Awarding Arc Points} %CHECK OUT AWARDING arc points for paragraph formatting
...
\end{longtabu}
\subsection{Spear Weapon Attachments} %MAKE subtable of spears?
%Weapon Name						&Weapon Type	&Hands	&Reach&Swing&Thrust&Defense (Guard)&Special						&Weight	&Cost\\\
...
\subsection{Polearm Weapon Attachments} %MAKE THIS A SUBTABLE OF THE POLEARMS
%Weapon Name						&Weapon Type	&Hands	&Reach&Swing&Thrust&Defense (Guard)&Special						&Weight	&Cost\\\
...
\paragraph{\label{wepqual:Parrying Teeth}Parrying Teeth}---\quad
...
\begin{table} [!hb] %CROSSBOWS LOOKS LIKE SHIT
	...
	\caption{Crossbows}
	\label{tab:Crossbows}
	\begin{tabu} to \linewidth {X[3] X *{4}{X[-2,c]}  X[4] *{2}{X[-2]}}
...
\chapter{Proficiencies}\label{ch:proficiencies} %PROFICIENCIES NEED DOING
\startcontents[chapters]
\printcontents[chapters]{}{1}{}
\clearpage
\begin{quote}
...
\rowfont[c]{}Weight& Encumbrance Level &CP Effect &Mobility Effect &Skill Penalty &Exhaustion Speed &Recover Speed\\\toprule
	...
	2$ \times $ CARRY& Medium &-2 &-4 &+2 RS &$ \times2 $ &$ \dfrac{1}{2}\times $\\
	3$ \times $ CARRY& Heavy &-3 &-6 &+3 RS &$ \times3 $ &$\dfrac{1}{4}\times $\\
...
\subsection{Outflanking} %OUTFLANKING
...
\subsection{Mounted Combat} %MOUNTED COMBTA
...
\subsection{Explosions}\label{sec:Explosions} %EXPLOSIONS
